...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
      {/* Top Header */}
      <div className="bg-gradient-to-r from-[#042038] to-[#083B66] text-white px-5 py-4 flex items-center justify-between border-b border-sky-900">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h3 className="font-bold text-base text-white">Student Gate Clearance Alerts</h3>
              {unreadCount > 0 && (
                <span className="bg-[#FAB582] text-[#052642] text-xs font-black px-2 py-0.5 rounded-full">
                  {unreadCount} New
                </span>
              )}
            </div>
            <p className="text-xs text-sky-200/90">
              Live automated SMS & Portal notifications sent upon gate scanning
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="text-sky-200 hover:text-white p-1 rounded-lg hover:bg-sky-900/60"
            >
              <X className="w-5 h-5" />
            </button>
          )}
      ...
      <div className="bg-sky-50/70 border-b border-sky-100 px-5 py-2.5 flex flex-wrap items-center justify-between text-xs text-[#083B66]">
